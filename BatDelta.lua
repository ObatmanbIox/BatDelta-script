-- BatDelta — Versão final: sistema de chaves + menu completo
...
local DATA_FILENAME = "batdelta_key_status.json" -- { users = { [userId]=expireTimestamp } }
local USED_FILENAME = "batdelta_used_keys.json" -- { [key]=true }
...
--------------------------------------------------------------------------------
-- Key GUI (Verificar + Link da chave + Pular Chave)
--------------------------------------------------------------------------------
...
-- notify server via RemoteEvent (if exists)
...
--------------------------------------------------------------------------------
-- Full menu implementation (complete, identical ao que validamos)
--------------------------------------------------------------------------------
local function createFullMenu(parent)
    -- Criar SG e elementos principais (layout e estilos conforme versão final)
    local SG = Instance.new("ScreenGui")
    SG.Name = "BatMenuGui"
    SG.ResetOnSpawn = false
    pcall(function() SG.IgnoreGuiInset = true end)
    ...
    BtnSim.Size = UDim2.fromOffset(140,36); BtnSim.Position = UDim2.new(0,20,0,88); BtnSim.BackgroundColor3 = Color3.fromRGB(60,200,90); BtnSim.Text = "Sim"; BtnSim.Font = Enum.Font.SourceSansBold; BtnSim.TextScaled = true; BtnSim.TextColor3 = Color3.new(1,1,1)
    BtnSim.ZIndex = 153
    local BtnNao = Instance.new("TextButton", Confirm)
    BtnNao.Size = UDim2.fromOffset(140,36); BtnNao.Position = UDim2.new(1,-160,0,88); BtnNao.BackgroundColor3 = Color3.fromRGB(220,60,60); BtnNao.Text = "Não"; BtnNao.Font = Enum.Font.SourceSansBold; BtnNao.TextScaled = true; BtnNao.TextColor3 = Color3.new(1,1,1)
    BtnNao.ZIndex = 153
    ...
    VP_Confirm.Size = UDim2.fromOffset(100,40); VP_Confirm.Position = UDim2.new(1, -110, 0, 112); VP_Confirm.BackgroundColor3 = Color3.fromRGB(60,200,90); VP_Confirm.Font = Enum.Font.SourceSansBold; VP_Confirm.TextScaled = true; VP_Confirm.Text = "confirmar"
    VP_Confirm.TextColor3 = Color3.new(1,1,1); VP_Confirm.ZIndex = 162

    local VP_Cancel = Instance.new("TextButton", ValuePrompt)
    VP_Cancel.Size = UDim2.fromOffset(100,40); VP_Cancel.Position = UDim2.new(0, 10, 0, 112); VP_Cancel.BackgroundColor3 = Color3.fromRGB(220,60,60); VP_Cancel.Font = Enum.Font.SourceSansBold; VP_Cancel.TextScaled = true; VP_Cancel.Text = "cancelar"
    VP_Cancel.TextColor3 = Color3.new(1,1,1); VP_Cancel.ZIndex = 162

    VP_Input.FocusLost:Connect(function(enter)
        if enter then VP_Confirm:CaptureFocus() end
    end)
    ...
    VP_Confirm.MouseButton1Click:Connect(function()
        ...
        if currentSetter and n then
            pcall(currentSetter, n)
        end
        ValuePrompt.Visible = false
        currentSetter = nil
    end)
    ...
    L1B.MouseButton1Click:Connect(function()
        safe("fly", function()
            ...
            if ok then
                ...
                -- show status on any existing keyGui safely
...
--------------------------------------------------------------------------------
-- Bootstrap: open key GUI and wire handlers
--------------------------------------------------------------------------------
...
function openKeyAndMenuBootstrap()
    ...
    -- LinkBtn: copia o novo link solicitado
